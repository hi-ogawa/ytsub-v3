...
export class AssertOkError extends Error {
  constructor(message?: string, stackStartFunction?: Function) {
    super(message);
    if ("captureStackTrace" in Error) {
      Error.captureStackTrace(this, stackStartFunction ?? AssertOkError);
    }
  }
}

export function assertOk(value: any, message?: string): asserts value {
  if (value) return;
  throw new AssertOkError(message, assertOk);
}
