...
export const loader: LoaderFunction = async function ({ request }) {
  ...
  const knexfile = require("../knexfile").migrations;
  return superjson.serialize({ data: request.url, knexfile, tables });
};
...
function Root() {
  const data = superjson.deserialize(useLoaderData());

  return (
    <div>
      <div>DATA</div>
      <div>{JSON.stringify(data, null, 2)}</div>
      <div>OUTLET START</div>
      <Outlet />
      <div>OUTLET END</div>
    </div>
  );
